    ...
    <form method="POST" action="{{route('products.update', ['product' => $product-> id] )}}">
        ...
        <div class="form-row">
            ...
            <Select class="custom-select" name="status" required>
                <option {{ old('status')  == 'avialable' ? 'selected' : ($product->status == 'available' ? 'selected' : '') }} value="available"> Available </option>
                <option {{ old('status')  == 'unavialable' ?  'selected'  : ( $product->status == 'unavailable' ? 'selected' : '' ) }} value="unavailable"> Unavailable </option>
            </Select>
        </div>
